...
class RegisterDietRegimes extends StatelessWidget {
  const RegisterDietRegimes({super.key});
                ...
                child: Obx(
                  () => Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Any Diet Regime followed earlier?',
                        style: TextStyle(
                          fontWeight: FontWeight.w700,
                          fontSize: 14,
                          color: glDarkPrimaryColor,
                      ...
                      Expanded(
                        child: SingleChildScrollView(
                          child: Wrap(
                            ...
                            children: RootService.to.config.value.dietRegimes
                                .mapIndexed(
                                  (dietRegime, i) => Container(
                                    margin: const EdgeInsets.symmetric(horizontal: 5, vertical: 2.5),
                                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
                                    decoration: BoxDecoration(
                                      color: authController.selectedDietRegimes.value.contains(dietRegime) ? glLightPrimaryColor : glLightDividerColor.withValues(alpha: 0.1),
                                      borderRadius: BorderRadius.circular(glBorderRadius),
                                      boxShadow: [
                                        BoxShadow(
                                          color: authController.selectedDietRegimes.value.contains(dietRegime) ? glLightThemeColor.withValues(alpha: 0.4) : glDarkPrimaryColor.withValues(alpha: 0.01),
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        InkWell(
                                          ...
                                          onTap: () {
                                            authController.openInfoPopup(title: dietRegime.title, description: dietRegime.details);
                                          },
                                        ).pOnly(right: 5),
                                        dietRegime.title.tr.text.size(12).make().onInkTap(() {
                                          if (!authController.selectedDietRegimes.contains(dietRegime)) {
                                            authController.selectedDietRegimes.add(dietRegime);
                                          } else {
                                            authController.selectedDietRegimes.remove(dietRegime);
                                          }
                                          authController.selectedDietRegimes.refresh();
                                        }),
                                        if (authController.selectedDietRegimes.contains(dietRegime))
                                          Icon(
                                            Icons.check_circle,
