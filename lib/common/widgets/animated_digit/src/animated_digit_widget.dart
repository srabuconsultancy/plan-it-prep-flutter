...
class _AnimatedDigitWidgetState extends State<AnimatedDigitWidget> with WidgetsBindingObserver {
  ...
  void didChangeDependencies() {
    ...
    if (_mediaQueryData?.textScaler != mq?.textScaler || _singleDigitData != sdp) {
      _markNeedRebuild();
    }
    _mediaQueryData = mq;
  ...
  String _formatNum(String numStr, {int fractionDigits = 2}) {
    String result;
    numStr = isNegative ? numStr.replaceFirst("-", "") : numStr;
    final List<String> numSplitArr = num.parse(numStr).toString().split('.');
        ...
        children: [
          ...
          if (widget.suffix != null)
            Transform.translate(
              ...
              child: _buildChangeTextColorWidget(widget.suffix!),
            ),
        ],
      ),
    );
  ...
  Widget _buildNegativeSymbol() {
    ...
    Widget secondChild = _singleDigitData?._buildChangeTextColorWidget("-", style, ValueKey(symbolKey)) ?? Text("-", key: ValueKey(symbolKey), style: style);
...
class _AnimatedSingleWidgetState extends State<_AnimatedSingleWidget> {
  ...
  Size _getTextSize(String text) {
    final fontWeight = PlatformDispatcher.instance.accessibilityFeatures.boldText
        ? FontWeight.bold
    ...
    TextPainter painter = TextPainter(
      ...
      textScaler: TextScaler.linear(widget.textScaleFactor ?? PlatformDispatcher.instance.textScaleFactor),
    );
    painter.layout();
    return painter.size;
  }
